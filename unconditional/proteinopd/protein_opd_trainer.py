...
STUDENT_PREFIX_INIT_METADATA_FILENAME = "student_prefix_init.json"
STUDENT_PREFIX_INIT_ADAPTER_DIRNAME = "student_prefix_adapter"
STUDENT_PREFIX_INIT_FORMAT = "protein_opd_student_prefix_init_v1"
...
class ProteinOPDTrainer(Trainer):
    """Geometric OPD trainer for unconditional ProtGPT2 generation."""
    ...
    def _save(self, output_dir: str | None = None, state_dict=None) -> None:
        ...
        if not hasattr(student_model, "save_pretrained") or not hasattr(student_model, "get_base_model"):
            raise ValueError("Prefix-initialized student model does not expose required save APIs.")

        base_model = student_model.get_base_model()
        safe_save = bool(getattr(self.args, "save_safetensors", False))
        save_kwargs = {"safe_" + "s" + "erialization": safe_save}
        base_model.save_pretrained(str(target_dir), **save_kwargs)

        adapter_dir = target_dir / STUDENT_PREFIX_INIT_ADAPTER_DIRNAME
        if adapter_dir.exists():
            shutil.rmtree(adapter_dir)
        student_model.save_pretrained(str(adapter_dir), **save_kwargs)

        tokenizer = getattr(self, "processing_class", None) or self._tokenizer
        if tokenizer is not None:
            tokenizer.save_pretrained(str(target_dir))

        metadata = {
            "format": STUDENT_PREFIX_INIT_FORMAT,
            "mode": "full_backbone_with_frozen_prefix",
            "adapter_relative_path": STUDENT_PREFIX_INIT_ADAPTER_DIRNAME,
            "prefix_trainable": False,
        }
        with open(target_dir / STUDENT_PREFIX_INIT_METADATA_FILENAME, "w", encoding="utf-8") as f:
            json.dump(metadata, f, indent=2, ensure_ascii=False)
        torch.save(self.args, target_dir / TRAINING_ARGS_FILENAME)
    ...
    def _assert_zero3_not_used(self) -> None:
        ...
        if getattr(deepspeed_plugin, "zero_stage", None) == 3:
            raise ValueError("Protein-OPD does not support DeepSpeed ZeRO-3.")
    ...
    def _canonicalize_rollouts(
        ...
        for row in generated_ids:
            ...
            while completion_ids and completion_ids[-1] == eos_token_id:
                completion_ids.pop()
            rollout_tensors.append(torch.tensor([eos_token_id] + completion_ids + [eos_token_id], dtype=torch.long, device=generated_ids.device))
        ...
        for idx, rollout in enumerate(rollout_tensors):
            ...
            labels[idx, 1:length] = rollout[1:]
        return rollout_batch, attention_mask, labels, rollout_tensors

    def _record_generation_outputs(self, rollout_tensors: list[torch.Tensor]) -> None:
        ...
        for rollout in rollout_tensors:
            ...
            if eos_token_id is not None and len(token_ids) >= 2 and token_ids[0] == eos_token_id and token_ids[-1] == eos_token_id:
                token_ids = token_ids[1:-1]
            sequence = self._tokenizer.decode(token_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)
            self._generation_outputs_buffer.append({"sequence": "".join(sequence.split())})
    ...
    def _compute_jsd_loss(
        ...
        if self.top_k_loss is not None and self.top_k_loss > 0:
            ...
            labels_for_loss = shifted_labels
        else:
            student_log_probs = student_log_probs_full
            target_log_probs = F.log_softmax(aggregated_teacher_log_probs, dim=-1)
            labels_for_loss = shifted_labels

        jsd_base_loss = self._generalized_jsd_loss_from_log_probs(student_log_probs, target_log_probs, labels=labels_for_loss, beta=self.distill_beta)
        entropy_term = None
        loss = jsd_base_loss
        if self.use_entropy_rule and self.entropy_alpha > 0:
    ...
    def compute_loss(
        ...
        self._record_generation_outputs(rollout_tensors)
