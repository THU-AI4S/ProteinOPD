...
class PeriodicSampleCallback(TrainerCallback):
    def __init__(self, trainer: "BaseProLLaMAOpdTrainer") -> None:
...
class BaseProLLaMAOpdTrainer(Trainer):
    ...
    def _sample_local_records(self, count: int) -> list[dict[str, str | None]]:
        if count <= 0:
            return []
            ...
            if self._wandb_enabled():
                try:
                    ...
                    if wandb.run is not None:
                        table = wandb.Table(columns=["step", "output_text", "sequence"])
                        for item in records:
                            table.add_data(step, item["output_text"], item["sequence"])
